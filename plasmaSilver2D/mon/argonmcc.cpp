...
void diagnos()
{
    ...
    for(int j=0; j< np[0]; j++) {
        double engy;
        engy = Exscale[0]*vx[0][j]*vx[0][j];
        engy+= Eyscale[0]*vy[0][j]*vy[0][j];
        engy+= Ezscale[0]*vz[0][j]*vz[0][j];
        e_mean+=engy;
    }
    printf("Emean=%f \n",e_mean/np[0]);
    }
...
void mcc_e_extra(int isp) //with extra energy
{
    ...
    if (init_mcc_flag) {
        ...
        if(ecollisional) {
            ...
            max_sigmav_e = 0.0;
            for (engy=0; engy</*ionengy0*/NEMAX; engy += 0.1)
            ...
            ecol_extra = 0.5;
            //printf("new %e \n", col_prob_e);//extra_e[int(frand()*np[0])],ecollisional,icollisional);
        }
        if(icollisional) {
            max_sigmav_i = 0.0;
            for (engy=0; engy<4/*NIMAX*/; engy += 0.1)
    ...
    if(ecollisional && isp==ecolsp) {
        ...
        for(j=0; j< np[ecolsp]; j++) {
            ...
            double sig1=sigma1(engy);
            double sig2=sigma2(engy);
            double sig3=sigma3(engy);

            double sigma_sum=sig1+sig2+sig3+1e-25;
            max_sigmav_e=sqrt(2*1.602e-19*engy/m[ecolsp])
                    *(sigma_sum);

            col_prob_e = 1 -exp(-max_sigmav_e*gden*dt*sp_k[ecolsp]);
            double ion_prob=col_prob_e*sig3/sigma_sum;
            extra_e[j]+=ion_prob;

            //  if (frand()>0.999)
            //  printf("new %e %e\n", col_prob_e,ion_prob);//extra_e[int(frand()*np[0])],ecollisional,icollisional);

            if (extra_e[j]>1.0) //emit the pair
            {
                extra_e[j]-=1.0;
...
void mcc(int isp) //honest
{
        ...
        if(ecollisional && isp==ecolsp) {
            ...
            for(j=0; j< np[ecolsp]; j++) {
               ...
               /* if (extra_e[j]>1.0) //emit the pair
                ...
                /*******************************/
                /* if the collision is elastic */
                if (random<col_prob_e)
                {

                    random=frand();
                    if (random <= sig1/sigma_sum) {
                    ...
                    /* if the collision is excitation */
                    ...
                    else if (engy >= extengy0 && random <= (sig1+sig2)/sigma_sum) {
                        /* first normalize vel */
                        vel  = sqrt(2.0*1.602e-19*engy/m[ecolsp]);
                        vx[ecolsp][j] *= vxscale[ecolsp]/vel;
                        vy[ecolsp][j] *= vyscale[ecolsp]/vel;
                    ...
                    else if(engy >= ionengy0 && random <= (sig1+sig2+sig3)/sigma_sum) {
                        /* first normalize vel */
                        vel  = sqrt(2.0*1.602e-19*engy/m[ecolsp]);
                        vx[ecolsp][j] *= vxscale[ecolsp]/vel;
                        vy[ecolsp][j] *= vyscale[ecolsp]/vel;
                        ...
                        /* note from djc, should double density at appropriate boundaries */	/* take diagnostics */
    ...
    //printf("icolsp=%d  isp=%d %d\n",icolsp,isp,icollisional);
    if(icollisional && isp==icolsp) {
      //  printf("icolsp=%d  isp=%d %d\n",icolsp,isp,icollisional);
        icol_extra += np[icolsp]*col_prob_i;
        N = icol_extra;
...
void mcc_vanilla(int isp) //vanilla
{
    diagnos();
...
void mcc_kostil0(int isp) //with extra energy
{
    ...
    if(ecollisional && isp==ecolsp) {
        for(j=0; j< np[ecolsp]; j++) {
            ...
            double k_e=250.0;
            if(engy*k_e>ionengy0) {
                //printf("engu=%f s1=%e s2=%e s3=%e \n",engy*k_e,sigma1(engy*k_e),sigma2(engy*k_e),sigma3(engy*k_e));
                sum_high++;
                max_sigmav_e=sqrt(2*1.602e-19*engy*k_e/m[ecolsp])
                        *(sigma1(engy*k_e)+sigma2(engy*k_e)+sigma3(engy*k_e));
                ...
                if(frand() < col_prob_e) {
                    ...
                    if (random <= (sum_sigma = sigma1(engy*k_e))/sigma_total) {
                        sum_rand1++;
                        /* first normalize vel */
                        vx[ecolsp][j] *= vxscale[ecolsp]/vel;
                        vy[ecolsp][j] *= vyscale[ecolsp]/vel;
                    ...
                    else if (engy*k_e >= extengy0 && random <= (sum_sigma +=sigma2(engy*k_e))/sigma_total) {
                        sum_rand2++;
                        /* first normalize vel */
                        vx[ecolsp][j] *= vxscale[ecolsp]/vel;
                        vy[ecolsp][j] *= vyscale[ecolsp]/vel;
                    ...
                    else if(engy*k_e >= ionengy0 && random <= (sum_sigma +=sigma3(engy*k_e))/sigma_total) {
                        sum_rand3++;
                        /* first normalize vel */
                        vx[ecolsp][j] *= vxscale[ecolsp]/vel;
                        vy[ecolsp][j] *= vyscale[ecolsp]/vel;
...
double sigma1(double energy)
{
    ...
    if(init_flag) {
        ...
        {
            int j=0;
            for (j=0; j<Ar_Els_num-1;j++)
            {
                if ((Ar_Els[j][0]<=i)&&(i<Ar_Els[j+1][0]))
                {
                    double alpha=(i-Ar_Els[j][0])/(Ar_Els[j+1][0]-Ar_Els[j][0]);
                    ioniz[i]=Ar_Els[j][1]*(1.0-alpha)+Ar_Els[j+1][1]*(alpha);
                    break;
                }
...
static double Ar_I[31][2] =
{{0.0000E+00, 0.0000E+00},
 {0.1580E+02, 0.0000E+00},
 {0.1600E+02, 0.2020E-21},
 {0.1700E+02, 0.1340E-20},
 {0.1800E+02, 0.2940E-20},
 {0.2000E+02, 0.6300E-20},
 {0.2200E+02, 0.9300E-20},
 {0.2375E+02, 0.1150E-19},
 {0.2500E+02, 0.1300E-19},
 {0.2650E+02, 0.1450E-19},
 {0.3000E+02, 0.1800E-19},
 {0.3250E+02, 0.1990E-19},
 {0.3500E+02, 0.2170E-19},
 {0.3750E+02, 0.2310E-19},
 {0.4000E+02, 0.2390E-19},
 {0.5000E+02, 0.2530E-19},
 {0.5500E+02, 0.2600E-19},
 {0.1000E+03, 0.2850E-19},
 {0.1500E+03, 0.2520E-19},
 {0.2000E+03, 0.2390E-19},
 {0.3000E+03, 0.2000E-19},
 {0.5000E+03, 0.1450E-19},
 {0.7000E+03, 0.1150E-19},
 {0.1000E+04, 0.8600E-20},
 {0.1500E+04, 0.6400E-20},
 {0.2000E+04, 0.5200E-20},
 {0.3000E+04, 0.3600E-20},
 {0.5000E+04, 0.2400E-20},
 {0.7000E+04, 0.1800E-20},
 {0.1000E+05, 0.1350E-20},
 {0.1000E+07, 0.1350E-20}};
...
double sigma3(double energy)
{
    ...
    if(init_flag) {
        ...
        {
            int j=0;
           for (j=0; j<Ar_I_num-1;j++)
           {
               if ((Ar_I[j][0]<=i)&&(i<Ar_I[j+1][0]))
               {
                   double alpha=(i-Ar_I[j][0])/(Ar_I[j+1][0]-Ar_I[j][0]);
                   ioniz[i]=Ar_I[j][1]*(1.0-alpha)+Ar_I[j+1][1]*(alpha);
                   break;
               }
